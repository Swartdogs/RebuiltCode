...
public final class TurretDirector
{
    ...
    private static TriangulationResult triangulate(TagObservation centerTagObservation, TagObservation offsetTagObservation)
    {
        ...
        double sinLcr      = (roDistanceMeters * Math.sin(angleLrcRad)) / ShooterConstants.TURRET_CL.in(Meters);
        double angleLcrRad = Math.asin(MathUtil.clamp(sinLcr, -1.0, 1.0));

        double angleLcrPlusNinetyRad = angleLcrRad + Math.PI / 2.0;
        double rhSquared             = (rcDistanceMeters * rcDistanceMeters) + (ShooterConstants.TURRET_CH.in(Meters) * ShooterConstants.TURRET_CH.in(Meters))
                - (2.0 * rcDistanceMeters * ShooterConstants.TURRET_CH.in(Meters) * Math.cos(angleLcrPlusNinetyRad));
        ...
        double sinHrc           = (ShooterConstants.TURRET_CH.in(Meters) * Math.sin(angleLcrPlusNinetyRad)) / rhDistanceMeters;
